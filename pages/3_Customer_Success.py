...
if True:
    ...
    with col1:
        ...
        prospect = st.selectbox(
            "Prospective Clients",
            dfm.clients[dfm.clients.account_status == "PROSPECT"].name,
        )
    ...
    with col2:
        customer = st.selectbox(
            "Current Clients",
            dfm.clients[dfm.clients.account_status == "CUSTOMER"].name,
        )
        ct = st.button("Get Client Summary")

    with col3:
        churned = st.selectbox(
            "Churned Clients", dfm.clients[dfm.clients.account_status == "CHURN"].name
        )
        cc = st.button("Get Ex-Client Summary")

    if passed_client:
        st_utils.print_client_details(dfm.clients[dfm.clients.name == qs_params['name'][0]], dfm)
    if pr:
        st_utils.print_client_details(dfm.clients[dfm.clients.name == prospect], dfm)

    if cc:
        st_utils.print_client_details(dfm.clients[dfm.clients.name == churned], dfm)

    if ct:
        st_utils.print_client_details(dfm.clients[dfm.clients.name == customer], dfm)
